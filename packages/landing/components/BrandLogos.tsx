// Real, official brand marks for the AI coding agents on the leaderboard and
// the /setup installer.
//
// Every mark is a self-contained inline <svg> — no external image or network
// requests, so nothing can 404 or flash. Path data is vendored from official
// sources (see SOURCES below) rather than approximated by hand:
//
//   • simple-icons (bundled build-time dep) — claude-code, gemini-cli,
//     github-copilot, cursor, windsurf, cline
//   • Sourced from each vendor's official assets — openai (OpenAI 2025 symbol),
//     amazon-q (AWS Toolkit official Q mark), continue (Continue repo mark)
//   • Monogram fallback — aider (ships only a font-rendered wordmark, no compact
//     glyph) and any unknown key.
//
// Marks render ~16-24px. Each carries the brand's official hex color. Near-black
// marks (OpenAI, GitHub Copilot, Cursor, Windsurf, Cline, Continue) adapt to the
// surface: their true near-black on the light /setup installer, and a lifted
// light-neutral on the dark home leaderboard, so they always meet contrast.
// They are decorative: callers pass the accessible name via surrounding text,
// and each <svg> carries a <title> for hover and is aria-hidden.
...
interface LogoProps {
  ...
  /** Extra classes. Color comes from the mark itself, not `currentColor`. */
  readonly className?: string;
  /**
   * Surface tone the logo renders on. Near-black brand marks (OpenAI, Copilot,
   * Cursor, Windsurf, Cline, Continue) use their true near-black on a light
   * surface and a lifted light-neutral on a dark surface, so they always stay
   * legible. Marks with a vivid brand color ignore this. Defaults to "dark"
   * (the leaderboard); the light /setup installer passes "light".
   */
...
/** Near-black brand marks lift to this light neutral on dark surfaces. */
const DARK_ON_DARK = "#e6eaf0";
/** ...and use this true near-black on light surfaces. */
const NEAR_BLACK = "#1b2128";

interface Mark {
  /** Accessible/title text. */
  readonly title: string;
  /** SVG viewBox (varies by source). */
  readonly viewBox: string;
  /**
   * Glyph color. A literal hex is used as-is on any surface (vivid brand
   * marks). The sentinel `"adaptive"` means the mark is near-black and should
   * be contrast-corrected per `surface`.
   */
  readonly color: string;
  /** Path `d` strings composing the glyph (filled with `color`). */
  readonly paths: readonly string[];
}

/** Sentinel marking a near-black glyph that adapts to the surface tone. */
const ADAPTIVE = "adaptive";

function resolveColor(color: string, surface: "light" | "dark"): string {
  if (color !== ADAPTIVE) return color;
  return surface === "light" ? NEAR_BLACK : DARK_ON_DARK;
}

// ── Vendored brand path data ──────────────────────────────────────────────
// Sources:
//   claude-code, gemini-cli, github-copilot, cursor, windsurf, cline:
//     simple-icons (npm `simple-icons`, viewBox 0 0 24 24)
//   openai:   File:OpenAI_logo_2025_(symbol).svg (Wikimedia Commons, viewBox 20)
//   amazon-q: aws/aws-toolkit-vscode q-gradient.svg (viewBox 128)
//   continue: continuedev/continue docs/logo mark (viewBox 26 24)
...
// Amazon Q — official gradient mark (AWS Toolkit). Rendered with the brand's
// cyan→violet linear gradient over a darker inner "Q". viewBox 0 0 128 128.
...
/** Render a single-color vendored mark. */
function SingleColorLogo(mark: Mark) {
  return function Logo({
    ...
    return (
      <svg
        {...base(size, mark.viewBox)}
        className={className}
        fill={resolveColor(mark.color, surface)}
      >
...
/** Amazon Q — official multi-color gradient mark. */
export function AmazonQLogo({ size = DEFAULT_SIZE, className }: LogoProps) {
  // Unique gradient id per render so multiple instances don't collide.
...
/**
 * Aider monogram fallback — Aider ships only a font-rendered wordmark, not a
 * compact glyph, so we use a clean "A" monogram in its brand green.
 */
export function AiderLogo({ size = DEFAULT_SIZE, className }: LogoProps) {
  return (
    <svg {...base(size, "0 0 24 24")} className={className}>
      ...
      <path
        d="M12 5.6 17.4 18h-2.46l-1.06-2.62H8.12L7.06 18H4.6L12 5.6Zm0 4.05-1.78 4.32h3.56L12 9.65Z"
        fill="#06140C"
      />
    </svg>
  );
...
/**
 * Neutral monogram fallback for unknown keys. A filled dark pill with a light
 * letter reads cleanly on both the dark leaderboard and light installer, so it
 * ignores `surface`.
 */
  ...
  return function Logo({ size = DEFAULT_SIZE, className }: LogoProps) {
    return (
      <svg {...base(size, "0 0 24 24")} className={className}>
        ...
        <rect x="1" y="1" width="22" height="22" rx="5.5" fill="#2a3340" />
        <text
          x="12"
          y="16.6"
          textAnchor="middle"
